...
class LEAR(object):
    ...
    def _build_and_split_XYs(self, df_train, df_test=None, date_test=None):
        ...
        if date_test is None:
            ...
            indexTest = df_test.loc[
                date_test : date_test + pd.Timedelta(hours=23)
            ].index
        ...
        for hour in range(24):
            indexTrain.loc[:, "h" + str(hour)] = indexTrain.index + pd.Timedelta(
                hours=hour
            )
            indexTest.loc[:, "h" + str(hour)] = indexTest.index + pd.Timedelta(
                hours=hour
            )
...
def evaluate_lear_in_test_dataset(
    ...
    for date in forecast_dates:
        ...
        smape = (
            np.mean(
                sMAPE(
                    forecast.loc[:date].values.squeeze(), real_values.loc[:date].values
                )
            )
            * 100
        )
